...
Use this reference when invoking `security-review` so the request quality is high and output is production-ready.
...
```text
Apply /security-review to the checkout area.
Context: Review application changes for authentication, authorization, data exposure, dependency risk, and operational abuse paths with concrete remediation guidance.
...
```text
Use /security-review for onboarding flow v2.
...
```text
Use /security-review and make it look cool.
```
