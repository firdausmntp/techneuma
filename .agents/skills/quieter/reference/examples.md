...
Use this reference when invoking `quieter` so the request quality is high and output is production-ready.
...
```text
Apply /quieter to the checkout area.
Context: Tone down overly bold or visually aggressive designs. Reduces intensity while maintaining design quality and impact.
...
```text
Use /quieter for onboarding flow v2.
...
```text
Use /quieter and make it look cool.
```
