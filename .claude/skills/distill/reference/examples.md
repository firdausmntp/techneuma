...
Use this reference when invoking `distill` so the request quality is high and output is production-ready.
...
```text
Apply /distill to the checkout area.
Context: Strip designs to their essence by removing unnecessary complexity. Great design is simple, powerful, and clean.
...
```text
Use /distill for onboarding flow v2.
...
```text
Use /distill and make it look cool.
```
