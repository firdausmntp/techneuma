...
Use this reference when invoking `flutter` so the request quality is high and output is production-ready.
...
```text
Apply /flutter to the checkout area.
Context: Expert Flutter development with Dart best practices, state management, and cross-platform patterns
...
```text
Use /flutter for onboarding flow v2.
...
```text
Use /flutter and make it look cool.
```
