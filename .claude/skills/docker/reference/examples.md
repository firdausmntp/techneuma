...
Use this reference when invoking `docker` so the request quality is high and output is production-ready.
...
```text
Apply /docker to the checkout area.
Context: Expert Docker and containerization patterns for development and production deployments
...
```text
Use /docker for onboarding flow v2.
...
```text
Use /docker and make it look cool.
```
