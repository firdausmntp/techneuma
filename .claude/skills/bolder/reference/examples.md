...
Use this reference when invoking `bolder` so the request quality is high and output is production-ready.
...
```text
Apply /bolder to the checkout area.
Context: Amplify safe or boring designs to make them more visually interesting and stimulating. Increases impact while maintaining usability.
...
```text
Use /bolder for onboarding flow v2.
...
```text
Use /bolder and make it look cool.
```
