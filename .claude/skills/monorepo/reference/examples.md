...
Use this reference when invoking `monorepo` so the request quality is high and output is production-ready.
...
```text
Apply /monorepo to the checkout area.
Context: Expert monorepo architecture with Turborepo, Nx, pnpm workspaces, and shared packages
...
```text
Use /monorepo for onboarding flow v2.
...
```text
Use /monorepo and make it look cool.
```
