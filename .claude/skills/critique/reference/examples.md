...
Use this reference when invoking `critique` so the request quality is high and output is production-ready.
...
```text
Apply /critique to the checkout area.
Context: Evaluate design effectiveness from a UX perspective. Assesses visual hierarchy, information architecture, emotional resonance, and overall design quality with actionable feedback.
...
```text
Use /critique for onboarding flow v2.
...
```text
Use /critique and make it look cool.
```
