...
Use this reference when invoking `websockets` so the request quality is high and output is production-ready.
...
```text
Apply /websockets to the checkout area.
Context: Expert WebSocket and real-time communication patterns with Socket.io, scaling, and reliability
...
```text
Use /websockets for onboarding flow v2.
...
```text
Use /websockets and make it look cool.
```
