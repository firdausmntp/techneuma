...
Use this reference when invoking `electron` so the request quality is high and output is production-ready.
...
```text
Apply /electron to the checkout area.
Context: Expert Electron development for cross-platform desktop applications with security and performance best practices
...
```text
Use /electron for onboarding flow v2.
...
```text
Use /electron and make it look cool.
```
