...
Use this reference when invoking `accessibility` so the request quality is high and output is production-ready.
...
```text
Apply /accessibility to the checkout area.
Context: Expert accessibility implementation with WCAG compliance, screen reader support, and inclusive design
...
```text
Use /accessibility for onboarding flow v2.
...
```text
Use /accessibility and make it look cool.
```
