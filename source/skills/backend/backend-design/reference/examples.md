...
Use this reference when invoking `backend-design` so the request quality is high and output is production-ready.
...
```text
Apply /backend-design to the checkout area.
Context: Design and refine backend services, APIs, jobs, and integration boundaries with production-grade contracts, failure handling, and operability.
...
```text
Use /backend-design for onboarding flow v2.
...
```text
Use /backend-design and make it look cool.
```
