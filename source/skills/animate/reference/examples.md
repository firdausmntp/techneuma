...
Use this reference when invoking `animate` so the request quality is high and output is production-ready.
...
```text
Apply /animate to the checkout area.
Context: Review a feature and enhance it with purposeful animations, micro-interactions, and motion effects that improve usability and delight.
...
```text
Use /animate for onboarding flow v2.
...
```text
Use /animate and make it look cool.
```
