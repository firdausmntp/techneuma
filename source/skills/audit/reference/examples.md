...
Use this reference when invoking `audit` so the request quality is high and output is production-ready.
...
```text
Apply /audit to the checkout area.
Context: Perform comprehensive audit of interface quality across accessibility, performance, theming, and responsive design. Generates detailed report of issues with severity ratings and recommendations.
...
```text
Use /audit for onboarding flow v2.
...
```text
Use /audit and make it look cool.
```
