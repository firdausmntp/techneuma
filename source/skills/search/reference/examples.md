...
Use this reference when invoking `search` so the request quality is high and output is production-ready.
...
```text
Apply /search to the checkout area.
Context: Expert search implementation with Elasticsearch, Algolia, Meilisearch, and full-text search patterns
...
```text
Use /search for onboarding flow v2.
...
```text
Use /search and make it look cool.
```
