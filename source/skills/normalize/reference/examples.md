...
Use this reference when invoking `normalize` so the request quality is high and output is production-ready.
...
```text
Apply /normalize to the checkout area.
Context: Normalize design to match your design system and ensure consistency
...
```text
Use /normalize for onboarding flow v2.
...
```text
Use /normalize and make it look cool.
```
