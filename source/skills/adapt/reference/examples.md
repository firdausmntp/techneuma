...
Use this reference when invoking `adapt` so the request quality is high and output is production-ready.
...
```text
Apply /adapt to the checkout area.
Context: Adapt designs to work across different screen sizes, devices, contexts, or platforms. Ensures consistent experience across varied environments.
...
```text
Use /adapt for onboarding flow v2.
...
```text
Use /adapt and make it look cool.
```
