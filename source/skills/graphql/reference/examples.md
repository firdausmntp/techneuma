...
Use this reference when invoking `graphql` so the request quality is high and output is production-ready.
...
```text
Apply /graphql to the checkout area.
Context: Expert GraphQL development with schema design, resolvers, and client integration patterns
...
```text
Use /graphql for onboarding flow v2.
...
```text
Use /graphql and make it look cool.
```
