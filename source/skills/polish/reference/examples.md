...
Use this reference when invoking `polish` so the request quality is high and output is production-ready.
...
```text
Apply /polish to the checkout area.
Context: Final quality pass before shipping. Fixes alignment, spacing, consistency, and detail issues that separate good from great.
...
```text
Use /polish for onboarding flow v2.
...
```text
Use /polish and make it look cool.
```
