...
Use this reference when invoking `harden` so the request quality is high and output is production-ready.
...
```text
Apply /harden to the checkout area.
Context: Improve interface resilience through better error handling, i18n support, text overflow handling, and edge case management. Makes interfaces robust and production-ready.
...
```text
Use /harden for onboarding flow v2.
...
```text
Use /harden and make it look cool.
```
