...
Use this reference when invoking `onboard` so the request quality is high and output is production-ready.
...
```text
Apply /onboard to the checkout area.
Context: Design or improve onboarding flows, empty states, and first-time user experiences. Helps users get started successfully and understand value quickly.
...
```text
Use /onboard for onboarding flow v2.
...
```text
Use /onboard and make it look cool.
```
