...
Use this reference when invoking `optimize` so the request quality is high and output is production-ready.
...
```text
Apply /optimize to the checkout area.
Context: Improve interface performance across loading speed, rendering, animations, images, and bundle size. Makes experiences faster and smoother.
...
```text
Use /optimize for onboarding flow v2.
...
```text
Use /optimize and make it look cool.
```
