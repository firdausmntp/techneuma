...
Body here.`;
		...
		expect(result.frontmatter.args[0].description).toBe(
...
Body.`;
	...
	test("should parse simple string arrays", () => {
		const content = `---
name: test-skill
domains:
  - backend
...
Body.`;
		...
		const result = parseFrontmatter(content);
		expect(result.frontmatter.domains).toEqual(["backend", "api", "services"]);
	});
...
describe("generateYamlFrontmatter", () => {
	...
	test("should roundtrip: generate and parse back", () => {
		const original = {
			...
			args: [{ name: "arg1", description: "First arg", required: true }],
		};
...
Audit the code.`;
	...
	test("should read skill with domains from frontmatter", () => {
		const skillContent = `---
name: backend-design
...
Design services carefully.`;
		...
		const skillDir = path.join(
			testRootDir,
			"source/skills/backend/backend-design",
		);
		ensureDir(skillDir);
		fs.writeFileSync(path.join(skillDir, "SKILL.md"), skillContent);
		...
		expect(skills).toHaveLength(1);
		expect(skills[0].domains).toEqual(["backend", "api", "services"]);
	});

	test("should ignore empty domains arrays so fallback logic can apply", () => {
		const skillContent = `---
name: backend-design
domains:
---
...
Body.`;
		...
		expect(skills[0].domains).toBeNull();
	});
...
Frontend design instructions.`;
		...
		ensureDir(refDir);
		fs.writeFileSync(
			path.join(refDir, "typography.md"),
			"Typography reference content.",
		);
...
describe("skill domain classification", () => {
	test("should prefer explicit domains from frontmatter", () => {
		const skill = {
			name: "backend-design",
			domains: ["custom", "domain"],
			sourcePath: path.join("backend", "backend-design"),
		};

		expect(getSkillDomains(skill)).toEqual(["custom", "domain"]);
	});

	test("should fall back to existing classification when domains are missing", () => {
		const skill = {
			name: "backend-design",
			sourcePath: path.join("backend", "backend-design"),
		};

		expect(getSkillDomains(skill)).toEqual(["backend", "api", "services"]);
	});

	test("shapeSkillForApi should expose resolved domains", () => {
		const shaped = shapeSkillForApi({
			name: "qa-strategy",
			description: "QA skill",
			userInvokable: false,
			domains: ["qa", "testing", "reliability"],
			references: [],
			sourcePath: path.join("qa", "qa-strategy"),
		});

		expect(shaped.domains).toEqual(["qa", "testing", "reliability"]);
	});
});

describe("replacePlaceholders", () => {
	test("should replace {{model}} with provider-specific value", () => {
		expect(replacePlaceholders("Ask {{model}} for help.", "claude-code")).toBe(
			"Ask Claude for help.",
		);
		expect(replacePlaceholders("Ask {{model}} for help.", "gemini")).toBe(
			"Ask Gemini for help.",
		);
		expect(replacePlaceholders("Ask {{model}} for help.", "codex")).toBe(
			"Ask GPT for help.",
		);
		expect(replacePlaceholders("Ask {{model}} for help.", "cursor")).toBe(
			"Ask the model for help.",
		);
		expect(replacePlaceholders("Ask {{model}} for help.", "agents")).toBe(
			"Ask the model for help.",
		);
		expect(replacePlaceholders("Ask {{model}} for help.", "kiro")).toBe(
			"Ask Claude for help.",
		);
	});

	test("should replace {{config_file}} with provider-specific value", () => {
		expect(replacePlaceholders("See {{config_file}}.", "claude-code")).toBe(
			"See CLAUDE.md.",
		);
		expect(replacePlaceholders("See {{config_file}}.", "cursor")).toBe(
			"See .cursorrules.",
		);
		expect(replacePlaceholders("See {{config_file}}.", "gemini")).toBe(
			"See GEMINI.md.",
		);
		expect(replacePlaceholders("See {{config_file}}.", "codex")).toBe(
			"See AGENTS.md.",
		);
		expect(replacePlaceholders("See {{config_file}}.", "agents")).toBe(
			"See .github/copilot-instructions.md.",
		);
		expect(replacePlaceholders("See {{config_file}}.", "kiro")).toBe(
			"See .kiro/settings.json.",
		);
	});
	...
	test("should exclude i-teach-impeccable from {{available_commands}}", () => {
		const result = replacePlaceholders(
			"Commands: {{available_commands}}",
			"claude-code",
			["i-audit", "i-teach-impeccable", "i-polish"],
		);
	...
	test("should replace multiple placeholders in the same string", () => {
		const result = replacePlaceholders(
			"{{model}} uses {{config_file}} and {{ask_instruction}}",
			"claude-code",
		);
		expect(result).toBe(
			"Claude uses CLAUDE.md and STOP and call the AskUserQuestion tool to clarify.",
		);
	});
...
describe("prefixSkillReferences", () => {
	test("should prefix /skillname command references", () => {
		const result = prefixSkillReferences("Run /audit to check.", "i-", [
			"audit",
			"polish",
		]);
	...
	test('should prefix "the skillname skill" references', () => {
		const result = prefixSkillReferences(
			"Use the audit skill for checks.",
			"i-",
			["audit", "polish"],
		);
	...
	test("should prefix multiple different references", () => {
		const result = prefixSkillReferences(
			"Run /audit then /polish. The audit skill is great.",
			"i-",
			["audit", "polish"],
		);
	...
	test('should handle case-insensitive "the X skill" matching', () => {
		const result = prefixSkillReferences("The audit skill is useful.", "i-", [
			"audit",
		]);
	...
	test("should not match /skillname inside longer words", () => {
		const result = prefixSkillReferences("The /auditing process.", "i-", [
			"audit",
		]);
	...
	test("should match /skillname before punctuation", () => {
		const result = prefixSkillReferences("Try /audit, /polish.", "i-", [
			"audit",
			"polish",
		]);
		expect(result).toContain("/i-audit,");
