...
Skills can be authored in either layout:
...
Rules:
- The build discovers `SKILL.md` files recursively anywhere under `source/skills/`.
- The directory containing `SKILL.md` is treated as the skill root.
- A sibling `reference/` directory is optional.
- Reference markdown files may also be nested, for example `reference/operations/observability.md`.
- Output providers still emit `.{config}/skills/{skill-name}/SKILL.md`, so nested source folders do not change installation paths.
...
```
source/                          → dist/
  skills/**/SKILL.md               cursor/.cursor/skills/{name}/SKILL.md
...
Reference files are copied into each provider skill directory under `reference/`, preserving nested subdirectories.
...
Or place it inside a domain folder:

```bash
mkdir -p source/skills/backend/myskill/reference
touch source/skills/backend/myskill/SKILL.md
```
...
- `readSourceFiles()`: Recursively reads `SKILL.md` files from `source/skills/` and loads sibling references
...
│   └── skills/                      # Skill definitions (flat or nested)
│       ├── frontend-design/
│       │   ├── SKILL.md
│       │   └── reference/*.md
│       ├── backend/
