...
Use this reference when invoking `authentication` so the request quality is high and output is production-ready.
...
```text
Apply /authentication to the checkout area.
Context: Expert authentication and authorization patterns including OAuth, JWT, sessions, and security best practices
...
```text
Use /authentication for onboarding flow v2.
...
```text
Use /authentication and make it look cool.
```
