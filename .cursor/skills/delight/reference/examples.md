...
Use this reference when invoking `delight` so the request quality is high and output is production-ready.
...
```text
Apply /delight to the checkout area.
Context: Add moments of joy, personality, and unexpected touches that make interfaces memorable and enjoyable to use. Elevates functional to delightful.
...
```text
Use /delight for onboarding flow v2.
...
```text
Use /delight and make it look cool.
```
