...
Use this reference when invoking `typeset` so the request quality is high and output is production-ready.
...
```text
Apply /typeset to the checkout area.
Context: Improve typography by fixing font choices, hierarchy, sizing, weight consistency, and readability. Makes text feel intentional and polished.
...
```text
Use /typeset for onboarding flow v2.
...
```text
Use /typeset and make it look cool.
```
