...
Use this reference when invoking `arrange` so the request quality is high and output is production-ready.
...
```text
Apply /arrange to the checkout area.
Context: Improve layout, spacing, and visual rhythm. Fixes monotonous grids, inconsistent spacing, and weak visual hierarchy to create intentional compositions.
...
```text
Use /arrange for onboarding flow v2.
...
```text
Use /arrange and make it look cool.
```
