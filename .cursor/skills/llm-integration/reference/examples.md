...
Use this reference when invoking `llm-integration` so the request quality is high and output is production-ready.
...
```text
Apply /llm-integration to the checkout area.
Context: Expert LLM/AI integration with OpenAI, Anthropic, local models, prompt engineering, and production patterns
...
```text
Use /llm-integration for onboarding flow v2.
...
```text
Use /llm-integration and make it look cool.
```
