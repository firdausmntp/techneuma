...
Use this reference when invoking `colorize` so the request quality is high and output is production-ready.
...
```text
Apply /colorize to the checkout area.
Context: Add strategic color to features that are too monochromatic or lack visual interest. Makes interfaces more engaging and expressive.
...
```text
Use /colorize for onboarding flow v2.
...
```text
Use /colorize and make it look cool.
```
