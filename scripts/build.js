...
function buildTailwindCSS() {
	...
	try {
		execSync(
			`bunx @tailwindcss/cli -i "${inputFile}" -o "${outputFile}" --minify`,
			{
				cwd: ROOT_DIR,
				stdio: "inherit",
			},
...
async function buildStaticSite() {
	...
	try {
		...
		console.log(
			`  JS: ${jsFiles.length} file(s) (${(jsFiles.reduce((s, f) => s + f.size, 0) / 1024).toFixed(1)} KB)`,
		);
		console.log(
			`  CSS: ${cssFiles.length} file(s) (${(cssFiles.reduce((s, f) => s + f.size, 0) / 1024).toFixed(1)} KB)`,
		);
...
function assembleUniversal(distDir, suffix = "") {
	...
	fs.writeFileSync(
		...
		`Impeccable - Multi-domain fluency for AI harnesses
https://impeccable.style
${prefixNote}
This folder contains the full multi-domain skill bundle for all supported tools:

  Core skills: frontend-design, backend-design, security-review
  Command layer: shared steering commands for quality, adaptation, enhancement, and system work
...
`,
	...
	console.log(
		`✓ Assembled universal${label} directory (${providerMappings.length} providers)`,
	);
}
...
function generateApiData(buildDir, skills, patterns) {
	...
	// skills.json
	const skillsData = skills.map(shapeSkillForApi);
	fs.writeFileSync(
	...
	// patterns.json
	fs.writeFileSync(
		path.join(apiDir, "patterns.json"),
	...
	console.log(
		`✓ Generated static API data (${skillsData.length} skills, ${commandsData.length} commands)`,
	);
}
...
`;
	...
	fs.writeFileSync(
		path.join(buildDir, "_routes.json"),
...
async function build() {
	...
	console.log(
		`📖 Read ${skills.length} skills (${userInvokableCount} user-invokable) and ${patterns.patterns.length + patterns.antipatterns.length} pattern categories\n`,
	);
