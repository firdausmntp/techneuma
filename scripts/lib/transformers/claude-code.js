...
export function transformClaudeCode(
	...
	for (const skill of skills) {
		...
		let skillBody = replacePlaceholders(
			skill.body,
			"claude-code",
			commandNames,
		);
		...
		if (skill.references && skill.references.length > 0) {
			...
			for (const ref of skill.references) {
				const refOutputPath = ref.relativePath
					? path.join(refDir, ref.relativePath)
	...
	console.log(
		`✓ Claude Code${prefixInfo}: ${skills.length} skills (${userInvokableCount} user-invokable)${refInfo}`,
	);
}
